    ...
    <section className="activities-workspace">
      ...
      <div className="panel activity-form-panel">
        ...
        <div className="form-grid">
          ...
          {activityForm.notify_team && <SelectField label="Equipe" value={activityForm.team_email} onChange={(value) => setActivityForm((current) => ({ ...current, team_email: value }))} options={[{ value: '', label: 'Selecione a equipe' }, ...context.teams.filter((team) => team.is_active && team.email).map((team) => ({ value: team.email!, label: `${team.name} - ${team.email}` })), ...(activityForm.team_email && !context.teams.some((team) => team.email === activityForm.team_email) ? [{ value: activityForm.team_email, label: `${activityForm.team_email} (equipe não cadastrada)` }] : [])]} />}
        </div>
  ...
  return (
    <Modal title={activity.name_snapshot} onClose={onClose} wide scrollable>
      ...
      {editing && <div className="form-grid">
        ...
        {draft.notify_team && <SelectField label="Equipe" value={draft.team_email} onChange={(value) => setDraft((current) => ({ ...current, team_email: value }))} options={[{ value: '', label: 'Selecione a equipe' }, ...context.teams.filter((team) => team.is_active && team.email).map((team) => ({ value: team.email!, label: `${team.name} - ${team.email}` })), ...(draft.team_email && !context.teams.some((team) => team.email === draft.team_email) ? [{ value: draft.team_email, label: `${draft.team_email} (equipe não cadastrada)` }] : [])]} />}
